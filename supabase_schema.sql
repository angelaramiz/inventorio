...
-- MIGRACIÓN 6: TEMPORADA DEFAULT EN CAJAS
-- Ejecuta esto en el SQL Editor de Supabase:
--
-- ALTER TABLE cajas ADD COLUMN IF NOT EXISTS temporada_default VARCHAR(100);
--
-- DROP VIEW IF EXISTS vista_total_cajas;
-- CREATE VIEW vista_total_cajas AS
-- SELECT 
...
-- LEFT JOIN zonas_almacen za_sec ON zs.id_zona_almacen = za_sec.id_zona_almacen
-- LEFT JOIN zonas_almacen za_direct ON c.id_zona_almacen = za_direct.id_zona_almacen
-- GROUP BY c.id_caja, c.numero_caja, c.sku, c.estado, c.fecha_creacion, c.id_zona_seccion, c.temporada_default, zs.nombre, c.id_zona_almacen, zs.id_zona_almacen, za_direct.nombre, za_sec.nombre;
